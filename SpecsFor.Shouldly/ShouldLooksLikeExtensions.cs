...
namespace SpecsFor.Shouldly
{
	public static class ShouldLooksLikeExtensions
	{
		...
		private static void ShouldMatchIEnumerable(IEnumerable actual, NewArrayExpression arrayExpression)
		{
			...
			array.Length.ShouldBe(arrayExpression.Expressions.Count, "Actual collection size differs from expected");
		...
		private static void ShouldMatchIEnumerable(IEnumerable actual, ListInitExpression arrayExpression)
		{
			...
			array.Length.ShouldBe(arrayExpression.Initializers.Count, "Actual collection size differs from expected");
		...
		private static void ShouldMatch(object actual, MemberInitExpression expression)
		{
			...
			foreach (var memberBinding in expression.Bindings)
			{
				...
				else if (IsSpecsForAnyExpression(bindingAsAnotherExpression))
				{
					...
					{
						throw new ShouldAssertException(Matcher.LastMatcher?.ToString());
					}
				}
				else
				{

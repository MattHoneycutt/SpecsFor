...
namespace SpecsFor.Shouldly.Tests.ShouldlyTests
{
    ...
    public class ShouldLookLikeExtensionsSpecs : SpecsFor<ShouldLookLikeExtensionsSpecs.TestObject>
	{
		...
		{
		    var equalException = Assert.Throws<ShouldAssertException>(() => SUT.ShouldLookLike(() => new TestObject
		    {
		        TestObjectId = Some.ValueOf<Guid>(x => x == Guid.Empty)
		    }));
		    equalException.Message.ShouldContain("Object matching (x == Guid.Empty)");
		}

		[Test]
		public void then_it_should_fail_when_used_with_a_not_default_matcher_that_does_not_match_a_specific_check()
		{
			Assert.Throws<ShouldAssertException>(() => new TestObject().ShouldLookLike(() => new TestObject
			{
				TestObjectId = Any.NonDefaultValueOf<Guid>()
			}))
			.Message.ShouldContain("Non-default value of System.Guid");
		}
		...
		public void then_it_should_fail_when_used_with_a_not_null_matcher_that_does_not_match_a_specific_check()
		{
			...
			.Message.ShouldContain("Non-null value of System.String");
		}
		...
		public void then_it_throws_when_a_null_value_for_a_nullable_type_is_not_allowed()
		{
			...
			Assert.Throws<ShouldAssertException>(() => SUT.ShouldLookLike(() => new TestObject
			{
		...
		public void then_it_should_fail_when_ienumerables_dont_match()
		{
			...
			Assert.Throws<ShouldAssertException>(() => list.ShouldLookLike(() => new[]
				{
					new TestObject
						{
							Name = "one",
